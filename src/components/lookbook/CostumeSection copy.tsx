...
import { CostumeList, Pagination, Select } from '@/components';
...
export default function CostumeSection({ selected, onSelect }: CostumeSectionProps) {
  ...
  const selectPart = (item: string) => {
    ...
    setKeyword('');
    if (item !== '부위') router.replace(`${pathname}?part=${COSTUME_PARTS.indexOf(item as CostumePart)}&page=1`);
  ...
  useEffect(() => {
    ...
    if (Number(pageParam) < 1 || isNaN(Number(pageParam))) {
      ...
      setPart(COSTUME_PARTS[Number(partParam)]);
